...
import { Employee, EmployeeRole, EmployeeStatus, EmploymentType } from "@/types/employee";
import Image from "next/image";
import { useEffect, useRef, useState } from "react";

type FormData = Omit<Employee, "id" | "employeeId" | "createdAt" | "lastLogin" | "permissions">;

const EMPTY: FormData = {
  firstName: "", lastName: "", email: "", phone: "",
  avatar: "", birthDate: "", gender: "male",
...
function Select({ value, onChange, children }: {
  ...
  return (
    <div className="relative">
      ...
      <svg className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-gray-400" width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M6 9l6 6 6-6" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
    </div>
  );
}

function SectionTitle({ children }: { children: React.ReactNode }) {
  return <h3 className="mb-3 mt-1 text-xs font-bold uppercase tracking-wider text-gray-400 dark:text-dark-6">{children}</h3>;
}
  ...
  return (
    <div className="flex items-center gap-4">
      <div className="relative h-16 w-16 overflow-hidden rounded-full bg-gray-100 dark:bg-dark-3 ring-2 ring-stroke dark:ring-dark-3">
        {value ? (
          <Image src={value} alt="avatar" fill sizes="64px" className="object-cover" />
        ) : (
      ...
      <div>
        ...
        <p className="mt-1 text-[11px] text-gray-400 dark:text-dark-6">JPG, PNG, WEBP · Max 2MB</p>
...
interface Props {
  open: boolean;
  onClose: () => void;
  onSave: (data: Omit<Employee, "id"|"employeeId"|"createdAt"|"lastLogin"> & { id?: string }) => void;
...
export function EmployeeSheet({ open, onClose, onSave, initial }: Props) {
  ...
  const handleSubmit = (ev: React.FormEvent) => {
    ev.preventDefault();
    if (!validate()) return;
    onSave({ ...form, permissions: initial?.permissions ?? [], ...(initial ? { id: initial.id } : {}) });
  };
  ...
  return (
    <>
      ...
      <aside role="dialog" aria-modal="true"
        className={cn("fixed right-0 top-0 z-9999 flex h-full w-full max-w-[560px] flex-col bg-white shadow-2xl transition-transform duration-300 ease-in-out dark:bg-gray-dark",
        ...
        <div className="flex items-center justify-between border-b border-stroke px-6 py-4 dark:border-dark-3">
          <div>
            ...
            <p className="mt-0.5 text-xs text-gray-400 dark:text-dark-6">
              {initial ? `ID: ${initial.employeeId}` : "Barcha kerakli maydonlarni to'ldiring"}
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-1 flex-col overflow-y-auto">
          <div className="flex flex-col gap-4 px-6 py-5">
            ...
            <AvatarUpload value={form.avatar} onChange={v => set("avatar", v)} />
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input value={form.firstName} onChange={v => set("firstName", v)} placeholder="Aziza" />
                {errors.firstName && <p className="mt-1 text-[11px] text-red-500">{errors.firstName}</p>}
              </div>
              <div>
                <Label required>Familiya</Label>
                <Input value={form.lastName} onChange={v => set("lastName", v)} placeholder="Karimova" />
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input value={form.phone} onChange={v => set("phone", v)} placeholder="+998 90 000 00 00" type="tel" />
                {errors.phone && <p className="mt-1 text-[11px] text-red-500">{errors.phone}</p>}
              </div>
              <div>
                <Label required>Email</Label>
                <Input value={form.email} onChange={v => set("email", v)} placeholder="aziza@hsse.uz" type="email" />
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <Label>Jins</Label>
                <Select value={form.gender} onChange={v => set("gender", v as "male"|"female")}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                {errors.branch && <p className="mt-1 text-[11px] text-red-500">{errors.branch}</p>}
              </div>
              <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input value={form.username} onChange={v => set("username", v)} placeholder="aziza.karimova" />
              </div>
              <div>
                <Label>Rol</Label>
                <Select value={form.role} onChange={v => set("role", v as EmployeeRole)}>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <Label>Status</Label>
                <Select value={form.status} onChange={v => set("status", v as EmployeeStatus)}>
          ...
          {/* Footer */}
          <div className="mt-auto flex gap-3 border-t border-stroke px-6 py-4 dark:border-dark-3">
